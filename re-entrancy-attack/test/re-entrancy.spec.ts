import { provider, expect, legos, parseUnits, formatUnits, wallet, DAI, GAS_LIMIT, vulnerableContract, attackerContract } from './setup';
...
describe('Attacker Contract', async function () {
  ...
  it('Wallet should deposit amt + gas to Attacker Contract', async function () {
    ...
    expect(Number(amtAttackerAfter)).to.be.closeTo(Number(amtAttackerBefore.add(ethAmountInGas.add(ethAmountIn))), Number(parseUnits('.05', legos.erc20.decimals)));
    
    let amtWalletAfter = await provider.getBalance(wallet.address);
    expect(Number(amtWalletAfter)).to.be.closeTo(Number(amtWalletBefore.sub(ethAmountIn.add(ethAmountInGas))), Number(parseUnits('.05', legos.erc20.decimals)));
